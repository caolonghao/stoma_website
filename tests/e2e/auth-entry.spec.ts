...
test("doctors can log out from their workspace", async ({ page }) => {
  await page.goto("/");
...
test("patients can log out from their workspace", async ({ page }) => {
  const uniqueSuffix = Date.now().toString().slice(-6);

  await page.goto("/");
  await page.getByRole("button", { name: "注册" }).click();
  await page.getByLabel("姓名").fill(`退出患者${uniqueSuffix}`);
  await page.getByLabel("手机号").fill(`139${uniqueSuffix.padStart(8, "0")}`);
  await page.getByLabel("密码").fill("Patient123!");
  await page.getByRole("button", { name: "创建账号" }).click();
